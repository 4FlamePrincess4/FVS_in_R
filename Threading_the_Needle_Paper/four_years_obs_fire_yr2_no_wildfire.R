...
#Name the FVS run
run_name <- str_c("obs_fire_2021_only",
...
create_fire1_kcp_files <- function(params_df, output_dir = "./fire2021_kcps", base_filename = "FlameAdjust") {
...
#Paths to kcps
fire1_kcps <- list.files(paste0(RunDirectory, "/fire2021_kcps"), full.names = TRUE)

#Create the .key and .in files for 36 runs (flame length fire 1 x flame length fire 2)

createInputFile_one_fire <- function(stand, managementID, outputDatabase, fire1_kcp, inputDatabase) {
  paste0(
    'STDIDENT\n', stand, '\n',
    'STANDCN\n', stand, '\n',
    'MGMTID\n', managementID, '\n',
...
write_keywords_parallel <- function(database_path, fire1_kcps) {
...
write_keywords_parallel(EC_dbs, fire1_kcps)
...
variants <- "ec"
fire1_kcps <- list.files(paste0(RunDirectory, "/fire2021_kcps"))
fire1_kcps <- stringr::str_sub(fire1_kcps, end = -5)
flame_lengths <- c(1, 3, 5, 7, 10, 20)
runs <- expand.grid(variant = variants, fire1_kcp = fire1_kcps)
#Add the flame lengths to each 
...
runFVS <- function(variant, fire1_kcp, flame_length1, RunDirectory, fvs_bin) {
  withr::with_dir(RunDirectory, {
    ...
    keyfile_path <- paste0(fire1_kcp, ".key")
