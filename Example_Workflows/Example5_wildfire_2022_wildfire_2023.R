...
setwd("C:/WFSETP/TTN_Paper/Treatment_Runs_FVS")
wd <- getwd()
...
TMFM2020_dir_path <- "C:/WFSETP/Data/TMFM_2020_OkaWen_Databases"

okawen_dbs <- list.files(TMFM2020_dir_path,
                         full.names=TRUE)

#Name the FVS run
run_name <- str_c("obs_fire_2022_wildfire_2023_all_variants",
...
create_fire1_kcp_files <- function(params_df, output_dir = "./fire2022_kcps", base_filename = "FlameAdjust") {
  ...
  for (i in seq_len(nrow(params_df))) {
    ...
    kcp_text <- paste0(
      ...
      "FLAMEADJ          ", p$fire1_year, "     PARMS(1.0, ", p$flame_length, ", CPC, HS_)\n",
      "!!simfire      year   wind(mph)  moisture    temp(F)  mortality  %burned   season\n",
      "SIMFIRE           ", p$fire1_year, "         ",
...
write_keywords_parallel <- function(database_paths, fire1_kcps, fire2_kcps) {
  for(db in seq_along(database_paths)){
    ...
    future_pmap(run_matrix, function(...){
      ...
      scenario_id <- paste0(fire1_name, "_", fire2_name, "_", standinit$Variant[1])
...
write_keywords_parallel(okawen_dbs, fire1_kcps, fire2_kcps)
...
runFVS <- function(variant, fire1_kcp, fire2_kcp, flame_length1, flame_length2, RunDirectory, fvs_bin) {
  withr::with_dir(RunDirectory, {
    ...
    keyfile_path <- paste0(fire1_kcp, "_", fire2_kcp, "_", toupper(variant), ".key")
...
#RunDirectory <- "C:/WFSETP/TTN_Paper/Treatment_Runs_FVS/obs_fire_2022_wildfire_2023_all_variants02Sep25_1436"
