...
setwd("C:/Users/Laurel/Documents/WFSETP/Scenarios/Maximum_Effort/FVS/FVS_Run4_tx_yr2_wildfire_yr3_10yrs")
wd <- getwd()
...
#Name the FVS run
run_name <- str_c("treat_plus_fire_ec_wc_variants",
...
write_keywords_parallel <- function(database_paths, FSim_scenarios, treat_kcps, fire_kcps) {
  for(db in seq_along(database_paths)){
    ...
    fire_df <- data.frame(fire_kcp = fire_kcps, flame_lengths = flame_lengths)
...
write_keywords_parallel(okawen_dbs, FSim_scenarios, treat_kcps, fire_kcps)
