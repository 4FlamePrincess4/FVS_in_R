...
setwd("C:/Users/Laurel/Documents/WFSETP/Scenarios/Maximum_Effort/FVS/FVS_Run2_notx_wildfire_yr3_10yrs")
wd <- getwd()
...
#Name the FVS run
run_name <- str_c("baseline_2020_fire_yr_3_FVS_run_all_variants",
...
write_keywords_parallel <- function(database_paths, FSim_scenarios, kcp_paths) {
  for(db in seq_along(database_paths)){
    ...
    future_pmap(run_matrix, function(...){
      ...
      scenario_id <- paste0("Scenario_", fl_label, "_", standinit$Variant[1])
...
write_keywords_parallel(okawen_dbs, FSim_scenarios, fire_kcps)
...
runFVS <- function(variant, fire_kcp, flame_length, RunDirectory, fvs_bin) {
  withr::with_dir(RunDirectory, {
    ...
    print(paste0("--keywordfile=Scenario_FL", flame_length, "_", toupper(variant), ".key"))
    rFVS::fvsSetCmdLine(
      cl = paste0("--keywordfile=Scenario_FL", flame_length, "_",  toupper(variant), ".key"),
