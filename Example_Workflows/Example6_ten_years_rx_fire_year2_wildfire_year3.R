...
#Name the FVS run
run_name <- str_c("rx_fire_yr2_wildfire_yr3_ec_wc_variants",
...
create_fire2_kcp_files <- function(params_df, output_dir = "./fire2022_kcps", base_filename = "FlameAdjust") {
  ...
  for (i in seq_len(nrow(params_df))) {
    ...
    kcp_text <- paste0(
      ...
      "FLAMEADJ          ", p$fire2_year, "     PARMS(1.0, ", p$flame_length, ", CPC, HS_)\n",
      "!!simfire      year   wind(mph)  moisture    temp(F)  mortality  %burned   season\n",
      "SIMFIRE           ", p$fire2_year, "         ",
...
#Paths to kcps
fire2_kcps <- list.files(paste0(RunDirectory, "/fire2022_kcps"), full.names = TRUE)
...
write_keywords_parallel <- function(database_paths, FSim_scenarios, treat_kcps, fire2_kcps) {
  for(db in seq_along(database_paths)){
    ...
    fire_df <- data.frame(fire_kcp = fire2_kcps, flame_lengths = flame_lengths)
    
    #Add the flame lengths to the run matrix
    run_matrix <- left_join(run_matrix, fire_df, by="fire_kcp")
...
write_keywords_parallel(okawen_dbs, FSim_scenarios, treat_kcps, fire2_kcps)
...
fire_scenarios <- list.files("./fire2022_kcps")
...
fire2_df <- data.frame(fire2_kcp = fire_scenarios, flame_length = flame_lengths)
runs <- dplyr::left_join(runs, fire2_df, by = "flame_length")
print(runs)
...
runFVS <- function(variant, flame_length, treatment, fire_kcp, RunDirectory, fvs_bin) {
  withr::with_dir(RunDirectory, {
    ...
    print(paste0("--keywordfile=",treatment, "_", fire_kcp, "_", variant, ".key"))
    rFVS::fvsSetCmdLine(
      cl = paste0("--keywordfile=",treatment, "_", fire_kcp, "_", variant, ".key"),
...
future::plan(multisession, workers = 32) #there are 30 runs so I'm going to set this lower to avoid crashing my computer
